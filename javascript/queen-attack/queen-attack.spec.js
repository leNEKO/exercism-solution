...
describe("Queens", function() {
    it("has the correct default positions", function() {
        ...
        expect(queens._white).toEqual([0, 3]);
        expect(queens._black).toEqual([7, 3]);
    });

    it("initialized with specific placement", function() {
        var queens = new Queens({ white: [3, 7], black: [6, 1] });
        expect(queens._white).toEqual([3, 7]);
        expect(queens._black).toEqual([6, 1]);
    });
    ...
    it("cannot occupy the same space", function() {
        ...
        try {
            ...
            expect(queens._white).toEqual([2, 4]);
            expect(queens._black).toEqual([2, 4]);
        } catch (error) {
