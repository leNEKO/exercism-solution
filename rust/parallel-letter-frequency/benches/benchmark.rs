...
use parallel_letter_frequency::{is_small_line_amounts, is_small_str_size};
use std::collections::HashMap;
use test::Bencher;

static LARGE_TEXT_SIZE: usize = 32;
...
#[bench]
fn bench_is_small_line_amount(b: &mut Bencher) {
    let texts = all_texts(LARGE_TEXT_SIZE);
    b.iter(|| is_small_line_amounts(&texts));
}

#[bench]
fn bench_is_small_str_size(b: &mut Bencher) {
    let texts = all_texts(LARGE_TEXT_SIZE);
    b.iter(|| is_small_str_size(&texts));
}
